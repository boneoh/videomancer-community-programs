...
--     101  LFSR synced   - Y OR frame-locked noise (>>4), gated by logical OR
--     110  PRNG          - Y OR line-seeded noise (>>4), gated by logical OR
--     111  Passthrough   - original Y/U/V pixel (no keying)
--
--   Show Matte (S1):
--     On  — S2/S3/S4 select the matte mode; the computed matte is output as Y,
--           with U and V forced to 512 (neutral chroma) for true monochrome.
--     Off — S2/S3/S4 are ignored; each channel gates independently.
--           Failing Y → 0; failing U/V → 512 (neutral chroma).  A full-range
--           channel (0-1023) always passes and drops out; only narrowed
--           channels gate.
--
...
architecture yuv_band_filter of program_top is
    ...
    --------------------------------------------------------------------------------
    -- Stage 0b: Pre-computed Y OR (noise >> 4) texture values (T+2)
    -- Computed alongside data registration so Stage 1a sees only registered signals.
    --------------------------------------------------------------------------------
    signal s_lfsr_tex_y_d1  : unsigned(C_VIDEO_DATA_WIDTH - 1 downto 0) := (others => '0');
    signal s_prng_tex_y_d1  : unsigned(C_VIDEO_DATA_WIDTH - 1 downto 0) := (others => '0');
    ...
    signal s_wf_lfsr_tex_y  : unsigned(C_VIDEO_DATA_WIDTH - 1 downto 0) := (others => '0');
    signal s_wf_prng_tex_y  : unsigned(C_VIDEO_DATA_WIDTH - 1 downto 0) := (others => '0');
...
begin
    ...
    begin
        if rising_edge(clk) then
            -- Pre-compute Y OR (noise >> 4) using registered pixel — no raw port fan-out.
            s_lfsr_tex_y_d1 <= unsigned(s_y_d1) or ("0000" & unsigned(s_lfsr(9 downto 4)));
            s_prng_tex_y_d1 <= unsigned(s_y_d1) or ("0000" & unsigned(s_prng(9 downto 4)));
    ...
    -- In p_window_key the Y is OR'd with (noise >> 4): Y occupies the full
    -- brightness range; the shifted noise adds at most 63/1023 of texture in
    -- the low bits.  Gated by logical OR in p_window_key.  Used by matte mode 101.
    --------------------------------------------------------------------------------
    p_lfsr : process(clk)
    ...
    begin
        if rising_edge(clk) then
            ...
            s_wf_lfsr_tex_y <= s_lfsr_tex_y_d1;
            s_wf_prng_tex_y <= s_prng_tex_y_d1;
        end if;
    ...
    --   "101" LFSR synced : Y OR frame-locked noise (>>4), gated by OR
    --   "110" PRNG        : Y OR line-seeded noise (>>4), gated by OR
    --   "111" Passthrough : original Y/U/V, no keying
    --
    -- Show Matte On  : S2/S3/S4 select matte mode; matte → Y; U=V=512 (neutral chroma)
    -- Show Matte Off : S2/S3/S4 ignored; per-channel gating; failing Y → 0, U/V → 512
        ...
        if rising_edge(clk) then
            if s_wf_avid = '0' then
                ...
                case s_wf_matte_mode is
                    ...
                    when "101" =>  -- LFSR synced: Y OR frame-locked noise (>>4), gated by OR
                        if s_wf_in_any = '1' then
                            v_matte := s_wf_lfsr_tex_y;
                        else
                            v_matte := (others => '0');
                        end if;
                    when "110" =>  -- PRNG: Y OR line-seeded noise (>>4), gated by OR
                        if s_wf_in_any = '1' then
                            v_matte := s_wf_prng_tex_y;
                        else
                            v_matte := (others => '0');
                        end if;
                ...
                if s_wf_show_matte = '0' then
                    -- Show Matte Off: S2/S3/S4 ignored; per-channel independent gating.
                    -- Failing Y → 0; failing U/V → 512 (neutral chroma).
                    -- Full-range channel (0-1023) always passes and drops out.
                    ...
                    -- Show Matte On, Passthrough: output original Y/U/V
                    s_processed_y <= unsigned(s_wf_y);
                    s_processed_u <= unsigned(s_wf_u);
                    s_processed_v <= unsigned(s_wf_v);
                else
                    -- Show Matte On: Y = matte; U/V = neutral chroma for true mono
