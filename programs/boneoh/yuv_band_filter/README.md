...
Matte Bit 2 (S2), Matte Bit 1 (S3), and Matte Bit 0 (S4) form a 3-bit word (S2 = MSB, S4 = LSB) that selects one of eight matte modes:
 ...
 ----   ----   ----  -------------- ------------- 
  ...
  1      0      1    LFSR           Matte = Y OR frame-locked noise (>>4), gated by logical OR  

  1      1      0    PRNG           Matte = Y OR line-seeded noise (>>4), gated by logical OR  
...
With Show Matte **Off** (S1 Off), switches S2, S3, and S4 are ignored entirely. Each channel gates independently: a passing Y outputs its original value; a failing Y outputs 0 (black luma). A failing U or V outputs 512 (neutral chroma). A channel whose window spans the full range (0–1023) always passes and effectively drops out — only the channels you narrow determine what appears in the output.

With Show Matte **On** (S1 On), S2, S3, and S4 select the matte mode. The computed matte is output as a greyscale signal: Y is set to the matte value, and U and V are forced to 512 (neutral chroma). This produces a true monochrome luma signal with no colour information — suitable for feeding other devices directly. Use this to dial in your thresholds visually, then switch Show Matte Off for the actual filtered output.

The Passthrough mode (S2=1, S3=1, S4=1) is only active when Show Matte is On; it outputs the original pixel on all channels.
...
**LFSR** — the matte is the Y value of the pixel OR'd with a 10-bit frame-locked noise value right-shifted by 4 (Fibonacci LFSR, polynomial x¹⁰ + x⁷ + 1). The LFSR is reseeded at each frame start from the current PRNG state so the pattern changes every frame. Y occupies the full brightness range (0–1023); the shifted noise adds at most 63/1023 of texture in the low bits. Gated by logical OR — the texture appears wherever any channel is in-window.

**PRNG** — the same Y OR (noise >> 4) approach as LFSR, but reseeded on every line start (from the LFSR state) rather than every frame. The line-based reseed produces a different spatial pattern from LFSR (which is frame-locked), creating a fine-grained texture that drifts continuously across frames. Gated by logical OR.
...
- **HD timing:** All six variants meet 74.25 MHz (worst case 76.39 MHz)
- **LC utilisation:** 3551–3577 of 7680 (~47%)
- **Package size:** 242,957 bytes (rev_b, unsigned)
