...
--     101  LFSR synced   - luma OR frame-locked noise (>>4), gated by logical OR
--     110  PRNG          - luma OR line-seeded noise (>>4), gated by logical OR
--     111  Passthrough   - original pixel on all channels (no keying)
--
--   Show Matte (S1):
--     On  — S2/S3/S4 select the matte mode; the computed matte is output as a
--           greyscale signal on all three channels (R=G=B → true monochrome YUV).
--     Off — S2/S3/S4 are ignored; each channel gates independently.
--           Failing channels output 0; a full-range channel (0-1023) always
--           passes and drops out — only narrowed channels gate.
--
...
begin
    ...
    -- In p_window_key the luma is OR'd with (noise >> 4): luma occupies the full
    ...
    -- both lines and frames.
    -- In p_window_key the luma is OR'd with (noise >> 4), matching LFSR mode 101.
    -- The line-based reseed produces a different spatial pattern from LFSR (which
    -- is frame-locked), creating a drifting fine-grained texture.
    ...
    --   "101" LFSR synced : luma OR frame-locked noise (>>4), gated by OR
    --   "110" PRNG        : luma OR line-seeded noise (>>4), gated by OR
    --   "111" Passthrough : original pixel, no keying
    --
    -- Show Matte On  : S2/S3/S4 select matte mode; matte output on all channels (greyscale)
    -- Show Matte Off : S2/S3/S4 ignored; per-channel gating, failing channels → 0
    --------------------------------------------------------------------------------
    ...
    begin
        if rising_edge(clk) then
            ...
            case s_wf_matte_mode is
                ...
                when "101" =>  -- LFSR synced: luma OR frame-locked noise (>>4), gated by OR
                    if s_wf_in_any = '1' then
                        v_matte := s_wf_luma or ("0000" & unsigned(s_wf_lfsr(9 downto 4)));
                    else
                        v_matte := (others => '0');
                    end if;
                when "110" =>  -- PRNG: luma OR line-seeded noise (>>4), gated by OR
                    if s_wf_in_any = '1' then
                        v_matte := s_wf_luma or ("0000" & unsigned(s_wf_prng(9 downto 4)));
                    else
                        v_matte := (others => '0');
                    end if;
            ...
            if s_wf_show_matte = '0' then
                ...
                -- Failing channel → 0.  Full-range channel (0-1023) always passes and
                -- drops out; only narrowed channels gate.
                if s_wf_in_r = '1' then s_processed_r <= s_wf_rgb_r; else s_processed_r <= (others => '0'); end if;
                if s_wf_in_g = '1' then s_processed_g <= s_wf_rgb_g; else s_processed_g <= (others => '0'); end if;
                if s_wf_in_b = '1' then s_processed_b <= s_wf_rgb_b; else s_processed_b <= (others => '0'); end if;
            elsif s_wf_matte_mode = "111" then
                -- Show Matte On, Passthrough: output original pixel on all three channels
                s_processed_r <= s_wf_rgb_r;
                s_processed_g <= s_wf_rgb_g;
                s_processed_b <= s_wf_rgb_b;
            else
                -- Show Matte On: greyscale matte on all channels (R=G=B → true mono YUV)
